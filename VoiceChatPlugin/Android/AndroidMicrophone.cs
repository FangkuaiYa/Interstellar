...
public class AndroidMicrophone : IDisposable
{
    ...
    public void Dispose()
    {
        ...
        _sourceAccumCount = 0;
        _lastStatus = "stopped";
        VoiceChatPluginMain.Logger.LogInfo($"{_logTag} Capture stopped.");
    }
    ...
    private bool EnsureStarted()
    {
        ...
        if (!StarlightVoiceNative.HasRecordAudioPermission())
        {
            if (!_permissionRequested)
            {
                ...
                VoiceChatPluginMain.Logger.LogInfo($"{_logTag} Requesting mic permission.");
            }
            ...
            {
                VoiceChatPluginMain.Logger.LogInfo($"{_logTag} Permission request sent.");
